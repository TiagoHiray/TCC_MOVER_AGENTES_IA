...
from hallucination import auditar_resultados
...
OPCOES_OLLAMA = {"num_ctx": 512, "temperature": 0.1}
...
def avaliar_modelo(modelo_nome: str, dataset: list[EventoLog]) -> list[ResultadoAvaliacao]:
    ...
    for evento in dataset:
        # Prompt atualizado utilizando a técnica Few-Shot para forçar formatação técnica
        prompt = (
            f"Você é um sistema de telemetria autônoma. Converta o diagnóstico e a ação em um resumo técnico e direto de UMA linha. "
            f"NÃO adicione explicações, saudações ou suposições mecânicas.\n\n"
            f"EXEMPLO 1:\n"
            f"Diagnóstico: Aceleração negativa excessiva detectada.\n"
            f"Ação Recomendada: Reduzir velocidade e aplicar controle de estabilidade.\n"
            f"Resumo: Risco de instabilidade por desaceleração brusca; velocidade reduzida e controle ativado.\n"
            f"-------------------\n"
            f"AGORA É A SUA VEZ:\n"
        ...
        inicio = time.time()
        resposta = ollama.generate(model=modelo_nome, prompt=prompt, options=OPCOES_OLLAMA)
        fim = time.time()
...
def executar_pipeline():
    ...
    for modelo in MODELOS_TESTE:
        try:
             todos_resultados.extend(avaliar_modelo(modelo, dataset))
    ...
    print(f"\n[OK] Métricas operacionais salvas em: {caminho_csv}")

    # Aciona a auditoria passando o CSV recém-criado
    df_alucinacoes = auditar_resultados(caminho_csv)
    
    resumo_tps = df_resultados.groupby('modelo')['throughput_tps'].agg(
        tps_min='min', tps_mean='mean', tps_max='max'
    ).reset_index()
    
    # Realiza a fusão das métricas e gera a tabela definitiva
    tabela_final = pd.merge(resumo_tps, df_alucinacoes, on='modelo', how='left')
    
    print("\n" + "="*85)
    print(" RESULTADO FINAL: THROUGHPUT vs ALUCINAÇÃO ".center(85))
    print("="*85)
    print(tabela_final.round(2).to_string(index=False))
    print("="*85)
